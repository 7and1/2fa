...
function normalizeOptions(options: TotpOptions = {}): NormalizedTotpOptions {
  const normalized: NormalizedTotpOptions = {
...
function getCachedSecretBytes(secret: string): Uint8Array {
...
function resolveHashAlgorithm(algorithm: HashAlgorithm = 'SHA-1'): HashAlgorithm {
  const upper = (algorithm || 'SHA-1').toUpperCase();
  return upper.startsWith('SHA-') ? upper : (`SHA-${upper}` as HashAlgorithm);
}

async function getCachedHmacKey(secretBytes: Uint8Array, secretKey: string, algorithm: HashAlgorithm): Promise<CryptoKey> {
...
async function hmacDigest(secretBytes: Uint8Array, normalizedSecret: string, counterBuffer: ArrayBuffer, algorithm: HashAlgorithm): Promise<Uint8Array> {
...
function truncate(hmacArray: Uint8Array): number {
...
export function getTimeWindow(period = 30, timestamp = Date.now(), epoch = 0): TimeWindow {
...
export async function verifyTOTP(secret: string, token: string, options: VerifyOptions = {}): Promise<boolean> {
...
function defaultBatchComparator(a: TotpEntry, b: TotpEntry): number {
  return a.issuer.localeCompare(b.issuer) || a.label.localeCompare(b.label);
}

export async function generateBatch(entries: TotpEntry[] = [], options: BatchOptions = {}): Promise<GeneratedToken[]> {
